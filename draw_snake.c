...
void Draw_Snake(Snake *snake,ALLEGRO_BITMAP *snake_body,ALLEGRO_BITMAP *snake_head,ALLEGRO_DISPLAY *display,double center_x,double center_y) {
	...
	int upBorder = center_y+displayH_2+(picture_size>>1);
	int downBorder = center_y-displayH_2-(picture_size>>1);
	int leftBorder = center_x-displayW_2-(picture_size>>1);
	int rightBorder = center_x+displayW_2+(picture_size>>1);


	for(i=0; i<snake->length; i++) {
		if(current_x<=rightBorder && current_x>=leftBorder && current_y<=upBorder && current_y>=downBorder) {
			if(current_body->prev==NULL) {
				Draw_Snake_Head(snake,center_x,center_y,picture_size,snake_head,display);
				return;
			} else {
...
//	LOG("theta = %f",theta*180/M_PI);
	...
	al_draw_scaled_rotated_bitmap(snake_head,SNAKE_GRAPH_SIZE/2,SNAKE_GRAPH_SIZE/2,Pos(snake->head->current_position.x,center_x,display,0),Pos(snake->head->current_position.y,center_y,display,1),scaled_rate,scaled_rate,theta,0);
